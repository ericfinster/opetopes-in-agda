...
module SliceMonad where
  ...
  module _ {ℓ} {I : Type ℓ} (M : PolyMonad I) where
    ...
    ρ-eqv sl-η {i , c} {lift unit} = {!!} ∘e lemma where
       lemma : Lift {j = ℓ} ⊤ ≃ SlPl (box c (λ p → ⟪ η M ⟫ lt) (λ p → dot (τ (P M) p)))
    ...
    sl-graft (dot i) δ₁ ε₁ = {!!} where -- transport! SlCn' (pair= (τ-coh (η M) lt) {!γ≈ (η-left-law M (γ-map (η M) lt))!}) (ε₁ (⟪ η M ⟫↓ lt))
      ηi : γ (P M) i
      ηi = ⟪ η M ⟫ lt
    ...
    sl-graft (box c δ ε) δ₁ ε₁ = transport! SlCn {!!} (box c (λ p → ⟪ μ M ⟫ (δ p , α p)) IH) where
      α : (p : ρ (P M) c) → (q : ρ (P M) (δ p)) → γ (P M) (τ (P M) q)
      α p q = {!!}
